...
exports.getAllRequests = async (req, res) => {
  try {
    ...
    const requests = await ServiceRequest.find({
      $or: [
        { provider_id: providerId }, // yêu cầu chỉ định
...
exports.completeRequest = async (req, res) => {
  try {
    ...
    // ✅ Tính total_amount tạm tính từ giá dịch vụ * diện tích
    let totalAmount = 0;
    ...
    if (provider?.services?.length > 0) {
      ...
      if (matchedService?.price) {
        totalAmount = (matchedService.price || 0) * (request.area_ha || 0);
      }
    }

    // ✅ Ghi vào trường `total_amount` trong request
